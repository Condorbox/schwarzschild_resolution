...
class GeodesicApp(tk.Tk):
    ...
    def __init__(self):
        ...
        self._result = None
        self._display_names = _preset_display_names()   # "Circular" -> "circular"
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        tk.Label(left, text="Schwarzschild", font=("Helvetica", 13, "bold"),
                 bg=PANEL_BG, fg=TEXT).pack(anchor="w", padx=14, pady=(14, 0))
        tk.Label(left, text="Geodesic Explorer", font=("Helvetica", 11),
                 bg=PANEL_BG, fg=TEXT_MUTED).pack(anchor="w", padx=14, pady=(0, 10))

        self._sep(left)

        # presets
        tk.Label(left, text="PRESETS", font=("Helvetica", 9, "bold"),
        ...
        for i, display_name in enumerate(self._display_names):
            b = tk.Button(btn_row, text=display_name, font=("Helvetica", 9),
                          bg=BORDER, fg=TEXT_MUTED, activebackground=ACCENT,
        ...
        # sliders
        tk.Label(left, text="PARAMETERS", font=("Helvetica", 9, "bold"),
                 bg=PANEL_BG, fg=TEXT_MUTED).pack(anchor="w", padx=14, pady=(8, 4))
        ...
        # integrator
        tk.Label(left, text="INTEGRATOR", font=("Helvetica", 9, "bold"),
                 bg=PANEL_BG, fg=TEXT_MUTED).pack(anchor="w", padx=14, pady=(8, 4))
        ...
        for s in ("RK4", "RK45", "DOP853"):
            rb = tk.Radiobutton(solver_row, text=s, variable=self._solver_var,
                                value=s, font=("Helvetica", 10),
        ...
        run_btn = tk.Button(left, text="▶  Integrate geodesic",
                            ...
                            bg=ACCENT, fg=BG, activebackground="#00b8d9",
                            activeforeground=BG, relief="flat", cursor="hand2",
                            padx=10, pady=8, command=self._run)
        ...
        for color, label in [(AMBER, "Photon sphere 1.5 rs"),
                             (PURPLE, "ISCO 3.0 rs"),
                             (GREEN, "Start"),
                             (PINK, "End"),
                             (ORANGE, "Plunge")]:
    ...
    def _mk_slider(self, parent, label, var, from_, to, fmt):
        ...
        header.pack(fill="x")
        tk.Label(header, text=label, font=("Helvetica", 10),
    ...
    def _mk_stat(self, parent, label: str) -> tk.Label:
        ...
        val = tk.Label(f, text="—", font=("Helvetica", 13, "bold"),
                       bg=PANEL_BG, fg=TEXT)
        val.pack(anchor="w")
        return val
    ...
    def _apply_preset(self, display_name: str):
        for n, b in self._preset_btns.items():
            b.config(bg=ACCENT if n == display_name else BORDER,
    ...
    def _redraw(self, sol):
        ...
        # event horizon  (RS = 2.0, so 1 rs unit on screen = 1 * scale)
        eh = 1.0 * scale
        ...
        cv.create_oval(sx-5, sy-5, sx+5, sy+5, fill=GREEN, outline="")
        ...
        if sol.plunged:
            ...
            cv.create_oval(ex-5, ey-5, ex+5, ey+5, fill=color, outline="")

        # r labels
        for gr in range(step, int(r_max) + step, step):
            lx, ly = _polar_to_xy(gr, math.pi * 0.25, cx, cy, scale)
            cv.create_text(lx, ly, text=f"{gr} rs",
                           fill=TEXT_MUTED, font=("Helvetica", 8))
